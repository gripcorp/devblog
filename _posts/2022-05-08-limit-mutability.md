...
### 요약정리
- `var`보다는 `val`을 이용해주세요.
- 뮤터블 객체보다는 이뮤터블 객체를 이용해주세요.
- 객체의 변경이 필요한 경우 이뮤터블 객체를 `data` 클래스로 만들고 `copy` 메소드를 이용해주세요.
- 변경점을 최소화 시키고 적절한 처리 및 테스트 코드를 작성해주세요.
- 이뮤터블 객체는 외부로 부터 최대한 숨겨주세요.
- 가끔은 효율성 때문에 뮤터블 객체를 이용하는 경우도 있지만, 최적화를 다시 한번 고려해주세요.
